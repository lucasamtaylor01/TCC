...
\section{Comparação entre os modelos} \label{sec:ch01_propriedades}
...
Os modelos analisados em \cite{Lorenz1980} devem satisfazer algumas condições. A primeira é a condição de dissipação:
\begin{equation}
...
A equação \eqref{eq:volume-zero} mostra que, como $B$ é positivo definido, o volume $V$ tende a zero quando o tempo avança. Isso implica que a dinâmica do sistema se restringe progressivamente a regiões de menor volume no espaço de fase. Qualquer superfície inicial $S$ gera uma sequência $S_1, S_2, \dots$, cada uma com volume menor que a anterior, até que, no limite, a sucessão de superfícies converge para um subconjunto de dimensão inferior, caracterizando um atrator dissipativo.

Relacionando esse modelo genérico aos modelos construídos, destacam-se dois casos. No modelo QG, a energia total é aproximadamente conservada pelos termos quadráticos, e a dissipação introduzida pelos processos de difusão atua de modo análogo ao termo dissipativo de \eqref{eq:sistema-generico}, de forma que o atrator possui volume zero. Já no modelo PE, embora a energia total não seja exatamente conservada, se $F_1^2+F_2^2+F_3^2$ for suficientemente pequeno, a condição \eqref{eq:condicao-sis} é satisfeita. Nesse caso, as trajetórias permanecem limitadas e o atrator também apresenta volume zero.
...
\subsection{Parâmetros} \label{subsec:parametros_e_variaveis}
...
\subsection{Evolução das variávies de indíce 1} \label{subsec:evolucao_variavel_1}
